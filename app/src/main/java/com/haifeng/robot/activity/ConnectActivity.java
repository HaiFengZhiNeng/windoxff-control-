...
public class ConnectActivity extends ActionBarActivity {
    ...
    private boolean initAccessory(final UsbDevice device) {
        ...
        connection.controlTransfer(0x40, 53, 0, 0, new byte[]{}, 0, Constant.USB_TIMEOUT_IN_MS);
    ...
    private void initStringControlTransfer(final UsbDeviceConnection deviceConnection,
                                           final int index,
                                           final String string) {
        deviceConnection.controlTransfer(0x40, 52, 0, index, string.getBytes(), string.length(), Constant.USB_TIMEOUT_IN_MS);
    }
}
