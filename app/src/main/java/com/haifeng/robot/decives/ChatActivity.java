package com.haifeng.robot.decives;
...
import com.haifeng.robot.activity.BaseChatActivity;
import com.haifeng.robot.activity.ConnectActivity;
    ...
    private class CommunicationRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (!claimResult) {
                ...
                final byte buff[] = new byte[Constant.BUFFER_SIZE_IN_BYTES];
                printLineToUI("Claimed interface - ready to communicate");

                while (keepThreadAlive.get()) {
                    final int bytesTransferred = connection.bulkTransfer(endpointIn, buff, buff.length, Constant.USB_TIMEOUT_IN_MS);
                    ...
                    synchronized (sendBuffer) {
                        if (sendBuffer.size()>0) {
                            ...
                            connection.bulkTransfer(endpointOut, sendBuff, sendBuff.length, Constant.USB_TIMEOUT_IN_MS);
                            sendBuffer.remove(0);
                        }
                    }
                }
